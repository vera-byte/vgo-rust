...
impl DatabaseManager {
    ...
    async fn build_any_pool(group: &str) -> Result<DbPool> {
        ...
        let url = match typ.as_str() {
            "postgresql" | "postgres" => match url_opt {
                ...
                None => {
                    let host: String =
                        mgr.get_or(&format!("database.{}.host", group), "127.0.0.1".to_string());
                    let port: String =
                        mgr.get_or(&format!("database.{}.port", group), "5432".to_string());
                    let user: String =
                        mgr.get_or(&format!("database.{}.user", group), "postgres".to_string());
                    let pass: String =
                        mgr.get_or(&format!("database.{}.pass", group), "".to_string());
                    let name: String =
                        mgr.get_or(&format!("database.{}.name", group), "postgres".to_string());
            ...
            "mysql" => match url_opt {
                ...
                None => {
                    let host: String =
                        mgr.get_or(&format!("database.{}.host", group), "127.0.0.1".to_string());
                    let port: String =
                        mgr.get_or(&format!("database.{}.port", group), "3306".to_string());
                    let user: String =
                        mgr.get_or(&format!("database.{}.user", group), "root".to_string());
                    let pass: String =
                        mgr.get_or(&format!("database.{}.pass", group), "".to_string());
                    let name: String =
                        mgr.get_or(&format!("database.{}.name", group), "mysql".to_string());
        ...
        let pool = match typ.as_str() {
            ...
            "postgresql" | "postgres" => return Err(anyhow!("未启用 postgres_backend feature")),
            #[cfg(feature = "mysql_backend")]
            "mysql" => {
            ...
            "sqlite" => return Err(anyhow!("未启用 sqlite_backend feature")),
            _ => return Err(anyhow!("不支持的数据库类型: {}", typ)),
        };
    ...
    async fn build_postgres_pool(group: &str) -> Result<Pool<ConnectionManager<PgConnection>>> {
        ...
        // 类型校验：必须为 postgresql
        let typ: String = mgr.get_or(
            &format!("database.{}.type", group),
            "postgresql".to_string(),
        );
        if typ.to_lowercase() != "postgresql" {
            return Err(anyhow!("数据库组 '{}' 类型非 postgresql: {}", group, typ));
        }
    ...
    pub async fn check_postgres_health(pool: &Pool<ConnectionManager<PgConnection>>) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            let mut conn: PooledConnection<ConnectionManager<PgConnection>> = pool
                .get()
                .map_err(|e: diesel::r2d2::PoolError| anyhow!(e))?;
    ...
    pub async fn check_mysql_health(pool: &Pool<ConnectionManager<MysqlConnection>>) -> Result<()> {
        let pool = pool.clone();
        tokio::task::spawn_blocking(move || {
            let mut conn: PooledConnection<ConnectionManager<MysqlConnection>> = pool
                .get()
                .map_err(|e: diesel::r2d2::PoolError| anyhow!(e))?;
            diesel::sql_query("SELECT 1")
                .execute(&mut conn)
    ...
    pub async fn check_sqlite_health(
        ...
        let pool = pool.clone();
        tokio::task::spawn_blocking(move || {
            let mut conn: PooledConnection<ConnectionManager<SqliteConnection>> = pool
                .get()
                .map_err(|e: diesel::r2d2::PoolError| anyhow!(e))?;
            diesel::sql_query("SELECT 1")
                .execute(&mut conn)
        ...
        match typ.as_str() {
            ...
            "postgresql" | "postgres" => {
                ...
                if let Err(e) = Self::check_postgres_health(&pool).await {
                    println!(
                        "[db] health failed for group '{}': {}; rebuilding",
                        group, e
                    );
            ...
            #[cfg(not(feature = "postgres_backend"))]
            "postgresql" | "postgres" => Err(anyhow!("未启用 postgres_backend feature")),
            #[cfg(feature = "mysql_backend")]
            "mysql" => {
                let pool = {
            ...
            #[cfg(not(feature = "sqlite_backend"))]
            "sqlite" => Err(anyhow!("未启用 sqlite_backend feature")),
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if let Err(e) = DatabaseManager::ensure_group_healthy(&group_name).await {
                    println!(
                        "[db] periodic health check error for '{}': {}",
                        group_name, e
                    );
                }
            }
    ...
    #[cfg(all(test, feature = "sqlite_backend"))]
    pub async fn pool_count_sqlite() -> usize {
        SQLITE_POOLS.read().await.len()
    }
    #[cfg(test)]
    pub async fn pool_count_any() -> usize {
        (&*ANY_POOLS).read().await.len()
    }

    /// 仅用于测试：重置池缓存
    #[cfg(all(test, feature = "postgres_backend"))]
    ...
    #[cfg(all(test, feature = "sqlite_backend"))]
    pub async fn reset_sqlite_pools() {
        SQLITE_POOLS.write().await.clear();
    }
    #[cfg(test)]
    pub async fn reset_any_pools() {
        (&*ANY_POOLS).write().await.clear();
    }
}
...
mod tests {
    ...
    fn test_build_postgres_url() {
        let url =
            DatabaseManager::build_postgres_url("10.0.0.200", "5432", "user@x", "p@ss:wd", "vgo");
