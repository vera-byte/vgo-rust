...
pub fn generate_model_artifacts(manifest_dir: &str, out_dir: &str) -> Result<()> {
    ...
    for rel in model_files.iter() {
        ...
        if let Some(mc) = gen_model_impl(&abs, rel) { model_code.push_str(&mc); }
    }
    ...
    for rel in model_files.iter() {
        ...
        m_files.entry(parent.to_path_buf()).or_default().push(rel.clone());
...
fn collect_rs_files(root: &Path, base: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in read_dir { if let Ok(e) = entry { let p = e.path(); if p.is_dir() { collect_rs_files(&p, base, out); } else if is_rs(&p) { if p.file_name().and_then(|s| s.to_str()) == Some("mod.rs") { continue; } let rel = p.strip_prefix(base).unwrap().to_path_buf(); out.push(rel); } } }
}

fn is_rs(p: &Path) -> bool { matches!(p.extension().and_then(|s| s.to_str()), Some("rs")) }

fn read_file(p: &Path) -> Option<String> { fs::read_to_string(p).ok() }

fn gen_model_impl(abs: &Path, rel: &PathBuf) -> Option<String> {
    let s = read_file(abs)?; let name = extract_struct_name(&s)?;
...
fn extract_struct_name(s: &str) -> Option<String> {
    let pat = "pub struct "; if let Some(i) = s.find(pat) { let tail = &s[i + pat.len()..]; let mut name = String::new(); for ch in tail.chars() { if ch.is_alphanumeric() || ch == '_' { name.push(ch); } else { break; } } if !name.is_empty() { return Some(name); } } None
}

fn extract_const(s: &str, key: &str) -> Option<String> { let pat = format!("pub const {}: &str =", key); if let Some(i) = s.find(&pat) { let tail = &s[i + pat.len()..]; if let Some(j) = tail.find('"') { let tail2 = &tail[j + 1..]; if let Some(k) = tail2.find('"') { return Some(tail2[..k].to_string()); } } } None }

fn default_table_name(rel: &PathBuf) -> String { let stem = rel.file_stem().and_then(|s| s.to_str()).unwrap_or("model"); format!("{}s", stem) }

fn to_model_module_path(p: &PathBuf) -> String { let mut parts: Vec<String> = vec!["crate".to_string(), "model".to_string()]; for c in p.components() { if let std::path::Component::Normal(os) = c { parts.push(os.to_string_lossy().replace(".rs", "")); } } parts.join("::") }

fn escape_path(p: &PathBuf) -> String { p.to_string_lossy().replace("\\", "/") }

fn emit_model_dir(out: &mut String, dir: &PathBuf, dirs: &BTreeMap<PathBuf, Vec<PathBuf>>, files: &BTreeMap<PathBuf, Vec<PathBuf>>) {
