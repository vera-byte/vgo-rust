//! 插件安装和加载模块 / Plugin installer and loader module
//!
//! 支持从 URL 下载 .wkp 插件包并安装
//! Support downloading and installing .wkp plugin packages from URL
...
impl PluginInstaller {
    ...
    pub fn install_from_file(&self, file_path: impl AsRef<Path>) -> Result<String> {
        ...
        let plugin_name = if buffer.starts_with(&[0x1f, 0x8b]) {
            ...
            // 可能是其他格式，暂时只支持 tar.gz
            // May be other format, currently only support tar.gz
            return Err(anyhow!(
    ...
    fn parse_plugin_info(&self, content: &str) -> Result<PluginInfo> {
        // 尝试 JSON / Try JSON
        ...
        // 尝试 YAML / Try YAML
        // 注意：这里简化处理，实际应该使用 yaml 解析库
        // Note: This is simplified, should use yaml parsing library in production
    ...
    pub fn list_installed(&self) -> Result<Vec<String>> {
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                if let Some(name) = path.file_name().and_then(|s| s.to_str()) {
                    // 跳过临时目录和套接字目录 / Skip temp and socket directories
...
mod tests {
    ...
    fn test_replace_variables() {
        let installer = PluginInstaller::new("/tmp");
        let url = "https://example.com/plugin-${os}-${arch}.wkp";
        let result = installer.replace_variables(url).unwrap();
        assert!(
            result.contains("darwin") || result.contains("linux") || result.contains("windows")
        );
    }
}
