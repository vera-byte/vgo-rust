...
#[utoipa::path(
    ...
    path = "/api/metrics",
    responses(
        (status = 200, description = "成功", body = crate::middleware::metrics::PerformanceMetrics)
    ),
    tag = "Metrics"
)]
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "成功", body = crate::api::swagger::MetricsReport)
    ),
    tag = "Metrics"
)]
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "成功", body = crate::api::swagger::ResetAck)
    ),
    tag = "Metrics"
)]
...
#[utoipa::path(
    ...
    path = "/api/health",
    responses(
        (status = 200, description = "健康", body = crate::api::swagger::HealthResponse),
        (status = 503, description = "不健康", body = crate::api::swagger::HealthResponse)
    ),
    tag = "Metrics"
)]
...
pub fn configure_metrics_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/api/metrics", web::get().to(get_metrics))
        .route("/api/metrics/report", web::get().to(get_performance_report))
        .route("/api/metrics/reset", web::post().to(reset_metrics))
        .route("/api/health", web::get().to(health_check_with_metrics));
}
